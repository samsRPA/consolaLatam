...
/* Demandante(s)/demandado(s) son SIEMPRE una lista (el bot puede traer varios actores
   del mismo tipo en actorsRama) -- se agrupa desde ahi en vez de usar el string ya unido
   por ";" que trae el proceso, para poder mostrar cada nombre en su propia linea cuando
   hay mas de uno. */
function actorNames(detail, tipo, fallback) {
  const actores = (detail && detail.actorsRama) || [];
  const names = actores
    .filter((a) => String(a.tipo_sujeto || "").toUpperCase() === tipo)
    .map((a) => a.nombre_actor)
    .filter(Boolean);
  if (names.length) return names;
  return fallback ? fallback.split(";").map((n) => n.trim()).filter(Boolean) : [];
}
function actorListHtml(names) {
  if (!names.length) return "—";
  if (names.length === 1) return esc(names[0]);
  return `<ul class="actor-list">${names.map((n) => `<li>${esc(n)}</li>`).join("")}</ul>`;
}
...
/* Paleta roja judicial para los gráficos. */
const CHART_PALETTE = ["#9a1413", "#c41e1e", "#e0605f", "#7a0f0e", "#f0a8a8", "#8a8a8a"];
...
async function loadDashboard() {
  let d;
  try { d = await api("/api/peru/dashboard"); } catch (err) { return showErrorPopup(err.message); }
  const stats = [
...
function drawLine(id, arr) {
  ...
  state.charts[id] = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: "Consultas", data: arr.map((x) => x.total), borderColor: cssVar("--forest"), backgroundColor: "rgba(154,20,19,.12)", fill: true, tension: .35, pointRadius: 3 },
...
async function renderCalendar() {
  ...
  let data;
  try { data = await api(`/api/peru/gestion/month?year=${year}&month=${month}`); } catch (err) { return showErrorPopup(err.message); }
...
async function selectDay(dateStr, cell) {
  ...
  let d;
  try { d = await api(`/api/peru/gestion/day?date=${dateStr}`); } catch (err) { showErrorPopup(err.message); panel.innerHTML = `<p class="muted">No se pudo cargar el día.</p>`; return; }
  ...
  $("#reminderForm").addEventListener("submit", async (e) => {
    ...
    try {
      await api("/api/peru/reminders", { method: "POST", body: form({
...
async function renderWeekAgenda(anchorDateStr = state.gestion.selectedDate) {
  ...
  let data;
  try {
    data = await Promise.all(dateKeys.map((date) => api(`/api/peru/reminders?date=${date}`)));
  } catch (err) {
...
function renderReminders(dateStr, reminders) {
  ...
  reminders.forEach((r) => {
    ...
    row.querySelector(".reminder__check").addEventListener("change", async (e) => {
      await api(`/api/peru/reminders/${r.id}/done`, { method: "PATCH", body: form({ done: e.target.checked }) });
      selectDay(dateStr, $(".calcell.is-selected"));
    });
    row.querySelector(".reminder__del").addEventListener("click", async () => {
      await api(`/api/peru/reminders/${r.id}`, { method: "DELETE" });
      selectDay(dateStr, $(".calcell.is-selected"));
      renderCalendar();
    });
...
$("#backupImportFile").addEventListener("change", () => {
  ...
  showConfirmPopup(
    ...
    async () => {
      try {
        await api("/api/peru/backup/import", { method: "POST", body: form({ file: input.files[0] }) });
        input.value = "";
...
async function loadClients() {
  state.clients = await api("/api/peru/clients");
  renderClients();
...
/* Selects de cliente en Inclusiones: a diferencia de fillClientSelect, muestran el
   Cliente ID de Oracle (para identificar cual es cual entre clientes con nombres
   parecidos) y, al elegir uno, muestran sus clientes hijos (jerarquia Oracle) debajo
   como checkboxes -- el cliente padre elegido en el select siempre va en la peticion al
   bot (opcional en Peru) y los hijos marcados se agregan al mismo arreglo `clientes`. */
...
async function loadIncChildren(sel) {
  ...
  let children = [];
  try { children = await api(`/api/peru/clients/${clientId}/children`); } catch (err) { return showErrorPopup(err.message); }
  ...
  ws.innerHTML = `
    ...
    <div class="dropzone" id="dropzone">
      ...
      <div class="dropzone__inner">
        ...
        <small>Detecta radicado · demandante · demandado automáticamente</small>
      </div>
    </div>
...
function deleteActiveClient() {
  showConfirmPopup(`¿Eliminar el cliente "${state.activeClient.name}" y todas sus bases? Esta acción no se puede deshacer.`, async () => {
    try {
      await api(`/api/peru/clients/${state.activeClient.id}`, { method: "DELETE" });
      state.activeClient = null;
...
async function loadHistory() {
  const list = await api(`/api/peru/clients/${state.activeClient.id}/history`);
...
async function loadClientChildren() {
  renderClientChildren(await api(`/api/peru/clients/${state.activeClient.id}/children`));
}
async function refreshClientChildren() {
  ...
  try {
    const list = await api(`/api/peru/clients/${state.activeClient.id}/children/refresh`, { method: "POST" });
...
async function loadBases() {
  const bases = await api(`/api/peru/clients/${state.activeClient.id}/bases`);
  renderBases(bases);
}
function mapTag(m) {
...
function deleteBase(id, name) {
  showConfirmPopup(`¿Eliminar la base "${name}"? Esta acción no se puede deshacer.`, async () => {
    try {
      await api(`/api/peru/bases/${id}`, { method: "DELETE" });
...
async function uploadFile(file) {
  ...
  try {
    const base = await api(`/api/peru/clients/${state.activeClient.id}/bases`, { method: "POST", body: form({ name: file.name.replace(/\.[^.]+$/, ""), file }) });
    await loadBases(); await loadClients();
...
async function loadProcesses() {
  ...
  let procs;
  try { procs = await api(`/api/peru/processes?${qs}`); } catch (err) { return showErrorPopup(err.message); }
...
function buildProcessCard(p, i) {
  ...
  card.innerHTML = `
    ...
    <div class="proccard__parties">
      <span><b>Demandantes:</b> ${actorListHtml(actorNames(p.detail, "DEMANDANTE", p.demandante))}</span>
      <span><b>Demandados:</b> ${actorListHtml(actorNames(p.detail, "DEMANDADO", p.demandado))}</span>
    </div>`;
...
async function enterProcess(processId) {
  ...
  let p;
  try { p = await api(`/api/peru/processes/${processId}`); } catch (err) { showErrorPopup(err.message); detail.innerHTML = `<p class="muted">No se pudo cargar el proceso.</p>`; return; }
  await api(`/api/peru/processes/${processId}/seen`, { method: "POST" }); // limpia marcas "nuevo"
  renderProcessDetail(p);
}
function renderProcessDetail(p) {
  const detail = $("#procDetail");
  const d = p.detail || {};
  const reporte = d.reporte || {};
  const despacho = d.despacho || p.organo || "";
  const valorParte = d.valorParte || "";
  const reporteRows = Object.entries(reporte).filter(([, v]) => v);
  const procesoId = d.procesoId;
  const botError = d.error || "";
  ...
  // peru_client.persist_case); sin eso PROCESOS_CLIENTES no tiene a que engancharse --
  // misma logica que Ecuador (ver app-ecuador.js::renderProcessDetail).
    ...
    <div class="procdetail__head">
      <div>
        ...
        <div class="procdetail__meta">
          ...
          <span><b>Demandantes:</b> ${actorListHtml(actorNames(d, "DEMANDANTE", p.demandante))}</span>
          <span><b>Demandados:</b> ${actorListHtml(actorNames(d, "DEMANDADO", p.demandado))}</span>
          ${despacho ? `<span><b>Despacho:</b> ${esc(despacho)}</span>` : ""}
          ${valorParte ? `<span><b>Valor parte:</b> ${esc(valorParte)}</span>` : ""}
          ${p.client_name ? `<span><b>Cliente:</b> ${esc(p.client_name)}</span>` : ""}
        </div>
      </div>
      <div class="procdetail__actions">
        <a class="btn btn--gold" href="/api/peru/processes/${p.id}/download">Descargar Excel</a>
        <button class="btn btn--danger-ghost" id="procDelete">Eliminar</button>
      </div>
    </div>
    ${botErrorBanner}
    <h3 class="ledger ledger--section">Datos del proceso</h3>
    <div class="acttable-wrap">
      ${reporteRows.length ? `<table class="acttable">
        <tbody>${reporteRows.map(([k, v]) => `<tr><td><b>${esc(k)}</b></td><td>${esc(v)}</td></tr>`).join("")}</tbody>
      </table>` : `<p class="muted">Sin datos adicionales del proceso.</p>`}
    </div>
    ${oracleClientesHtml}`;
  ...
  $("#procDelete").addEventListener("click", () => {
    showConfirmPopup(`¿Eliminar el proceso ${p.radicado}? Esta acción no se puede deshacer.`, async () => {
      try {
        await api(`/api/peru/processes/${p.id}`, { method: "DELETE" });
...
async function loadProcOracleClientes(processId) {
  ...
  let list;
  try { list = await api(`/api/peru/processes/${processId}/oracle-clientes`); }
...
async function buscarOracleCliente(processId) {
  ...
  try {
    procOracleLookup = await api(`/api/peru/oracle-clientes/${clienteId}/lookup`);
  } catch (err) {
    procOracleLookup = null;
...
async function agregarOracleClientesAlProceso(processId) {
  ...
  try {
    const result = await api(`/api/peru/processes/${processId}/oracle-clientes`, {
...
$("#downloadAllProc").addEventListener("click", async () => {
  const clientId = $("#procClientFilter").value;
  const url = `/api/peru/processes-download${clientId ? "?client_id=" + clientId : ""}`;
  try {
...
$("#deleteAllProc").addEventListener("click", () => {
  ...
  showConfirmPopup(msg, async () => {
    try {
      const result = await api(`/api/peru/processes${clientId ? "?client_id=" + clientId : ""}`, { method: "DELETE" });
...
/* ============================================================
   INCLUSIONES
   ============================================================ */
/* Documento (tipoDocumento/numeroDocumento/codigo/fechaEmision/fechaNacimiento) es
   OPCIONAL: si los 5 campos quedan vacios el servidor manda un documento por defecto
   (ver app.py:DEFAULT_DOCUMENTO); en cuanto se llena alguno, se exige el resto con las
   mismas reglas de siempre. Se comparte entre Individual y Masivo. */
function readDocumentoFields(prefix) {
  // El checkbox "Elegir datos del documento" oculta el formulario por defecto; si esta
  // apagado se ignora lo que haya quedado escrito (residuo de un toggle previo) y se
  // manda todo vacio, para que el servidor use el documento por defecto sin excepciones.
  if (!$(`#${prefix}DocumentoToggle`).checked) {
    return { tipoDocumento: "", numeroDocumento: "", codigo: "", fechaEmision: "", fechaNacimiento: "" };
  }
  return {
    tipoDocumento: $(`#${prefix}TipoDocumento`).value,
    numeroDocumento: $(`#${prefix}NumeroDocumento`).value.trim(),
    codigo: $(`#${prefix}Codigo`).value.trim(),
    fechaEmision: $(`#${prefix}FechaEmision`).value,
    fechaNacimiento: $(`#${prefix}FechaNacimiento`).value,
  };
}
function validateDocumentoFields(d) {
  const { tipoDocumento, numeroDocumento, codigo, fechaEmision, fechaNacimiento } = d;
  const algunoLleno = tipoDocumento || numeroDocumento || codigo || fechaEmision || fechaNacimiento;
  if (!algunoLleno) return null; // nada lleno: el servidor usa el documento por defecto
  if (!tipoDocumento) return "El tipo de documento es obligatorio si vas a llenar el documento";
  if (!numeroDocumento) return "El número de documento es obligatorio si vas a llenar el documento";
  if (!/^\d+$/.test(numeroDocumento)) return "El número de documento debe ser numérico";
  if (!fechaEmision) return "La fecha de emisión es obligatoria si vas a llenar el documento";
  if (!fechaNacimiento) return "La fecha de nacimiento es obligatoria si vas a llenar el documento";
  if (tipoDocumento === "DNI") {
    if (numeroDocumento.length !== 8) return "El DNI debe tener 8 dígitos numéricos.";
    if (!codigo) return "El código es obligatorio";
    if (!/^\d+$/.test(codigo)) return "El código debe ser numérico";
  } else if (tipoDocumento === "CE") {
    if (numeroDocumento.length < 9 || numeroDocumento.length > 12) {
      return "El Carné de Extranjería debe tener entre 9 y 12 dígitos.";
    }
  }
  const hoy = new Date().toISOString().slice(0, 10);
  if (fechaEmision > hoy) return "La fecha de emisión no puede ser una fecha futura";
  if (fechaNacimiento > hoy) return "La fecha de nacimiento no puede ser una fecha futura";
  if (fechaNacimiento > fechaEmision) return "La fecha de nacimiento no puede ser posterior a la fecha de emisión";
  return null;
}

$("#incTipoDocumento").addEventListener("change", (e) => {
  const esDni = e.target.value === "DNI";
  $("#incCodigoWrap").hidden = !esDni;
  if (!esDni) $("#incCodigo").value = "";
});
$("#incBulkTipoDocumento").addEventListener("change", (e) => {
  const esDni = e.target.value === "DNI";
  $("#incBulkCodigoWrap").hidden = !esDni;
  if (!esDni) $("#incBulkCodigo").value = "";
});
$("#incDocumentoToggle").addEventListener("change", (e) => { $("#incDocumentoFields").hidden = !e.target.checked; });
$("#incBulkDocumentoToggle").addEventListener("change", (e) => { $("#incBulkDocumentoFields").hidden = !e.target.checked; });

$("#inclusionForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  const radicado = $("#incRadicado").value.trim();
  const demandante = $("#incDemandante").value.trim();
  const demandado = $("#incDemandado").value.trim();
  const valorParte = $("#incValorParte").value.trim();
  const clientId = $("#incClient").value;
  if (!radicado) return toast("El radicado es obligatorio");
  if (!demandante) return toast("El demandante es obligatorio");
  if (!demandado) return toast("El demandado es obligatorio");
  if (!clientId) return toast("Selecciona un cliente");
  const documento = readDocumentoFields("inc");
  const docError = validateDocumentoFields(documento);
  if (docError) return toast(docError);
  try {
    const run = await api("/api/peru/inclusiones", { method: "POST", body: form({
      radicado, demandante, demandado, valor_parte: valorParte, client_id: clientId,
      tipo_documento: documento.tipoDocumento, numero_documento: documento.numeroDocumento, codigo: documento.codigo,
      fecha_emision: documento.fechaEmision, fecha_nacimiento: documento.fechaNacimiento,
      hijo_ids: selectedIncChildren("#incChildren"),
    }) });
    openConsole({ name: radicado, row_count: 1 }, "inclusion", run.id);
    streamRun(run.id);
  } catch (err) { showErrorPopup(err.message); }
});

function resetInclusionForm() {
  $("#incRadicado").value = ""; $("#incDemandante").value = ""; $("#incDemandado").value = ""; $("#incValorParte").value = "";
  $("#incTipoDocumento").value = ""; $("#incNumeroDocumento").value = ""; $("#incCodigo").value = "";
  $("#incCodigoWrap").hidden = true; $("#incFechaEmision").value = ""; $("#incFechaNacimiento").value = "";
}
$("#incClearBtn").addEventListener("click", resetInclusionForm);
...
/* carga masiva por Excel dentro de Inclusiones */
...
async function bulkInclude(file) {
  ...
  const documento = readDocumentoFields("incBulk");
  const docError = validateDocumentoFields(documento);
  if (docError) return toast(docError);
  toast("Cargando base y detectando columnas…", true);
  try {
    const base = await api(`/api/peru/clients/${clientId}/bases`, { method: "POST", body: form({ name: file.name.replace(/\.[^.]+$/, ""), file }) });
    $("#incFileInput").value = "";
    showConfirmPopup(`Se detectaron ${base.row_count} radicados. ¿Iniciar la consulta TOTAL y crear las cards?`, async () => {
      try {
        const run = await api(`/api/peru/bases/${base.id}/run`, { method: "POST", body: form({
          mode: "total", hijo_ids: hijoIds,
          tipo_documento: documento.tipoDocumento, numero_documento: documento.numeroDocumento, codigo: documento.codigo,
          fecha_emision: documento.fechaEmision, fecha_nacimiento: documento.fechaNacimiento,
        }) });
        openConsole({ name: base.name, row_count: base.row_count }, "total", run.id);
        streamRun(run.id);
        loadClients();
      } catch (err) { showErrorPopup(err.message); }
    }, { okLabel: "Sí, iniciar" });
...
async function loadNotifications() {
  let data;
  try { data = await api(`/api/peru/notifications?notif_filter=${state.notifFilter}`); } catch (err) { return showErrorPopup(err.message); }
...
function buildNotif(n) {
  ...
  if (rb) rb.addEventListener("click", async (e) => {
    e.stopPropagation();
    await api(`/api/peru/notifications/${n.id}/resolve`, { method: "POST" });
  ...
  if (fileInput) fileInput.addEventListener("change", async (e) => {
    ...
    try {
      await api(`/api/peru/notifications/${n.id}/attachment`, { method: "POST", body: form({ file: fileInput.files[0] }) });
  ...
  row.querySelector(".notif__body").addEventListener("click", async () => {
    if (!n.read) await api(`/api/peru/notifications/${n.id}/read`, { method: "PATCH", body: form({ read: true }) });
...
async function runGlobalSearch() {
  ...
  let data;
  try { data = await api(`/api/peru/search?q=${encodeURIComponent(q)}`); } catch (err) { showErrorPopup(err.message); wrap.innerHTML = `<p class="muted">No se pudo buscar.</p>`; return; }
  const items = [];
...
$("#notifReadAll").addEventListener("click", async () => { await api("/api/peru/notifications/read-all", { method: "POST" }); loadNotifications(); });
$("#notifDeleteSel").addEventListener("click", async () => {
  if (!state.notifSelection.size) return;
  await api("/api/peru/notifications/delete", { method: "POST", body: form({ ids: [...state.notifSelection].join(",") }) });
  loadNotifications();
...
$("#notifDeleteAll").addEventListener("click", () => {
  showConfirmPopup("¿Eliminar TODAS las notificaciones? Esta acción no se puede deshacer.", async () => {
    try {
      await api("/api/peru/notifications", { method: "DELETE" });
      loadNotifications();
      toast("Notificaciones eliminadas", true);
    } catch (err) { showErrorPopup(err.message); }
  });
...
function startRun(base, mode) {
  const label = mode === "total" ? "TOTAL (consulta completa)" : "DIARIA (consulta completa)";
  // Segundo paso de confirmacion antes de arrancar el navegador y el recorrido.
  showConfirmPopup(
    `¿Iniciar la consulta ${label} de la base "${base.name}"?\n\nSe enviará al servicio de consultas y se creará una card por proceso.`,
    async () => {
      try {
        const run = await api(`/api/peru/bases/${base.id}/run`, { method: "POST", body: form({ mode }) });
...
function openConsole(base, mode, runId) {
  ...
  $("#liveParte").textContent = "Consultando con el servicio externo…";
  $("#liveDone").textContent = "0";
  $("#liveTotal").textContent = base.row_count;
  // El bot resuelve todo en una sola llamada bloqueante (no hay progreso caso-por-caso
  // real mientras se espera), asi que arranca en modo indeterminado -- igual que Ecuador
  // -- y se vuelve una barra normal en cuanto empiezan a llegar resultados.
...
$("#consoleCancel").addEventListener("click", () => {
  ...
  showConfirmPopup("¿Detener esta consulta? El expediente en curso terminará y los demás se cancelan.", async () => {
    ...
    try { await api(`/api/peru/runs/${runId}/cancel`, { method: "POST" }); }
...
function streamRun(runId) {
  ...
  const es = new EventSource(`/api/peru/runs/${runId}/stream`);
...
function handleEvent(ev, runId) {
  switch (ev.type) {
    case "case_started":
      // OJO: no se quita is-indeterminate aca. En una inclusion individual este evento
      // se publica ANTES de llamar al bot (no cuando ya hay resultado), asi que si se
      // quitara aca la animacion se cancelaria casi al instante y quedaria un rojo
      // solido fijo durante toda la espera real. Se quita recien en case_done, que es
      // cuando de verdad hay un resultado.
...
function addLog(ev) {
  ...
  const statusText = ok ? "✓ OK" : "✕ " + esc(friendlyError(ev.error) || "error");
  line.innerHTML = `<span class="logline__idx">${ev.index}/${ev.total}</span><span class="logline__rad">${esc(ev.radicado)}</span><span class="logline__status ${ok ? "ok" : "err"}" title="${esc(ev.error || "")}">${statusText}</span>`;
...
function finishConsole(ev, runId) {
  ...
  if (ev.status === "done") {
    if (isInclusion) {
      ...
      if (ev.process_ids) {
        $("#downloadBtn").href = `/api/peru/processes-download-batch?ids=${ev.process_ids}`;
      ...
      $("#downloadBtn").href = `/api/peru/runs/${runId}/download`;
      $("#downloadBtn").hidden = false;
    }
    ...
    // El popup queda afuera aca a propósito: la consola ya es su propio dialogo
    // abierto (con este mismo mensaje adentro); abrir el popup encima solo apila dos
    // fondos oscuros uno sobre otro. console.error() igual deja rastro para depurar.
    console.error("[Consola CEJ]", ev.error);
...
async function updateRetryButton(runId) {
  ...
  try {
    const data = await api(`/api/peru/runs/${runId}/cases`);
...
$("#retryErrorsBtn").addEventListener("click", () => {
  ...
  showConfirmPopup("¿Reconsultar solo los procesos que fallaron en esta corrida?", async () => {
    try {
      const run = await api(`/api/peru/runs/${sourceRunId}/retry-errors`, { method: "POST" });
...
async function loadSchedules(baseId) {
  const list = await api(`/api/peru/bases/${baseId}/schedules`);
  ...
  list.forEach((s) => {
    ...
    li.querySelector("[data-toggle]").addEventListener("click", async () => { await api(`/api/peru/schedules/${s.id}`, { method: "PATCH", body: form({ enabled: !s.enabled }) }); loadSchedules(baseId); });
    li.querySelector("[data-delsched]").addEventListener("click", async () => { await api(`/api/peru/schedules/${s.id}`, { method: "DELETE" }); loadSchedules(baseId); });
    ul.appendChild(li);
  });
}
$("#scheduleForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  try {
    await api(`/api/peru/bases/${state.scheduleBaseId}/schedules`, { method: "POST", body: form({ mode: $("#schedMode").dataset.value, hour: $("#schedHour").value, minute: $("#schedMinute").value }) });
...
async function refreshNotifBadge() {
  try { const d = await api("/api/peru/dashboard"); updateNotifBadge(d.notifications.no_leidas); } catch (_) {}
}
loadClients()
